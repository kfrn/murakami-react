...
const CartItem = ({bookDetails, cartItem, dispatch}) => {
  return (
    <div className='cart-item'>
      ...
      <div className='cart-book-name'><Link to={`/books/${bookDetails.id}`}>{bookDetails.title}</Link></div>
      ...
      <div className='add-remove'>
        <button className='cart-button' onClick={() => dispatch(incrementQuantity(bookDetails.id))}>Add another</button>
        <br/>
        <button className='cart-button' onClick={() => dispatch(removeFromCart(bookDetails.id))}>Remove</button></div>
    </div>
  );
}
